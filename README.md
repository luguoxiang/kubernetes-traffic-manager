...
echo $INGRESS_HOST
# if it is empty, try INGRESS_HOST=`kubectl get svc traffic-ingress -o jsonpath='{.status.loadBalancer.ingress[0].ip}'`
...
Ensure your traffic-ingress service has a public loadbalancer hostname(${INGRESS_HOST}). For example, in AWS EKS cluster, it will be XXX.elb.amazonaws.com.
```
mkdir certbot
